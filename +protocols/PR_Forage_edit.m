classdef PR_Forage_edit < handle
  ...
  methods (Access = public)
    function o = PR_Forage_edit(winPtr)
      o.winPtr = winPtr;
    ...
    function initFunc(o,S,P);
       ...
       o.Bars = stimuli.barcode(o.winPtr,'bkgd',S.bgColour,'gray',false); % 1D noise as probe
       o.Bars.prctgray = 33.33;
       
       o.Bars.texnum = 1;
       o.Bars.Ntex = 20000;
       o.Bars.barwidth  = round(P.barwidth*S.pixPerDeg);
       o.Bars.radius    = round(P.proberadius*S.pixPerDeg);
...
%         end
    ...
    function reset_probe_location_and_texture(o,firstime)
        ...
        o.Bars.position = [(o.S.centerPix(1) + round(o.PosList(1)*o.S.pixPerDeg)),...
    ...
    function [drop,faceitem] = updateProbes(o,xx,yy,currentTime)
        ...
        if (o.state > 0)
            ...
            if (o.MovStep > 0)
                ...
                o.Bars.position = [(o.S.centerPix(1) + round(newPos(1)*o.S.pixPerDeg)),...
                                                (o.S.centerPix(2) - round(newPos(2)*o.S.pixPerDeg))];
                if (o.state == 3)
                    o.Faces.position = o.Bars.position;
                end
                %***************************
            end
        end
        ...
        if (o.PFrameCount < o.MaxFrame)
            ...
            if (drop == 1)  % will be moving to state 2 or 3
                   ...
                   o.ProbeHistory(o.PFrameCount,4) = NaN;
                ...
                if (o.state < 2)
                   ...
                   o.ProbeHistory(o.PFrameCount,4) = o.Bars.texnum;
