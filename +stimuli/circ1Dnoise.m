classdef circ1Dnoise < stimuli.stimulus % inherit stimulus to have tracking / random number generator
  ...
  methods (Access = public)
    function o = circ1Dnoise(winPtr,varargin) % marmoview's initCmd?
      o.winPtr = winPtr;
      o.tex = [];
      o.texDim = [];
      o.saveline = [];
      ...
      if isempty(o.pixPerDeg)
            warning('circ1DStim: I need the pixPerDeg to be accurate')
            o.pixPerDeg = 37.5048;
      end
    ...
    %             pause(0.01)
        ...
        elseif distr==3
            ...
            filt = filt+.01; %Pink space/time+white
            filt(isinf(filt))=0;
            ...
            switch o.sparsity
                ...
                case 2
                    line=M2;
                otherwise
                    error('circ1Dnoise: Unknown sparsity value')
            end
...
%             o.tex(i) = Screen('MakeTexture',o.winPtr,rim);
            o.tex(i) = Screen('MakeTexture',o.winPtr, ...
                cat(3,repmat(im0(:,:,i),1,1,3),t1));
%             o.tex(i) = Screen('MakeTexture',o.winPtr, ...
%                 cat(3,repmat(im0(round(dim/2),:,i),1,1,3),t1(round(dim/2),:)));
        end
