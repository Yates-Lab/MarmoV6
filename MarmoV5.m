...
function MarmoV5_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    % TODO: should go back to the old way with reward
    % -------------------------------------------------------------------------
    % --- Feedback (e.g., reward system, sound, etc.)
    numFeedback = numel(S.feedback);
    handles.feedback = cell(numFeedback,1);
    for i = 1:numFeedback
        assert(ismember(handles.S.feedback{i}, fieldnames(handles.S)), 'MarmoV5.m line 158: requested outputs needs parameter struct field of that name')
        handles.feedback{i} = marmoview.(handles.S.feedback{i})(handles.S.(handles.S.feedback{i}));
    end

    % TODO: reward volume displayed in GUI??
    if numel(handles.feedback) >  0 && isprop(handles.feedback{1}, 'volume')
        handles.A.juiceVolume = handles.feedback{1}.volume;
    else
...
function ClearSettings_Callback(hObject, eventdata, handles)
    ...
    dx = handles.A.dx;
    dy = handles.A.dy;
    if ~handles.S.DummyEye % TODO: how do we want to handle calibration?
...
function RunTrial_Callback(hObject, eventdata, handles)
    ...
    for i = 1:numel(handles.intputs)
        handles.intputs{i}.pause();
    end
    %******************************
